...
engine=create_engine("sqlite:///ww_cigarette_database.db")
...
def socioeconomic_2012():
    session=Session(engine)
    results=session.query(smoke.CountryName, smoke.Year,smoke.PercentageOfSmokersInPopulation,smoke.GDPPerCapita,smoke.Unemployment,smoke.HealthExpenditure).\
        filter(smoke.Year==2012).all()
    session.close()
    
    year_socio_data=[]
    for CountryName, Year, PercentageOfSmokersInPopulation, GDPPerCapita, Unemployment, HealthExpenditure in results:
        socio_dict={}
        socio_dict["Country"]=CountryName
        socio_dict["Year"]=Year
        socio_dict["Percentage Total"]=PercentageOfSmokersInPopulation
...
#def socioeconomic_all_years():
    #session=Session(engine)
    #results=session.query(smoke.Country_id, smoke.CountryName, smoke.Year,smoke.PercentageOfSmokersInPopulation,smoke.GDPPerCapita,smoke.Unemployment,smoke.HealthExpenditure).all()
    #session.close()
    
    #all_socio_data=[]
    #for Country_id, CountryName, Year, PercentageOfSmokersInPopulation, GDPPerCapita, Unemployment, HealthExpenditure in results:
        #socio_dict={}
        #socio_dict["Country"]=CountryName
        #socio_dict["Year"]=Year
        #socio_dict["Percentage Total"]=PercentageOfSmokersInPopulation
